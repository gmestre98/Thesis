...
numHiddenUnits = {283}; %TEST THIS NUMBER OF UNITS
DelayX = 1; %TEST THIS DELAY
folder_list = {'Experiment1/TDNN-PTRS/283/Run10/'};

for foldernum=1:length(folder_list)

    net = timedelaynet(1:DelayX, numHiddenUnits{foldernum});
    ...
    [XsValidation, XiValidation, AiValidation, YsValidation] = ...
         preparets(net, XTotValidation, YTotValidation);

    YcValidation = net(XsValidation, XiValidation, AiValidation);
    perval{foldernum} = perform(net, YsValidation, YcValidation) 
    YYY = net(XsTrain, XiTrain, AiTrain);
    pertrain{foldernum} = perform(net, YsTrain, YYY)
    for k=1:10
      %% DB1
      figure(1+4*(k-1))
      hold on
...
%     end
    ...
    [XsTest, XiTest, AiTest  YsTest] = ...
         preparets(net, XTotTest, YTotTest);
    
    YcTest = net(XsTest, XiTest, AiTest);
    pertest{foldernum} = perform(net, YsTest, YcTest);
    for k=1:10
      % DB1
      figure(41+4*(k-1))
      hold on
